...
const getChartData = async (config) => {
  chartDataList.value = []
  ...
  chartDataList.value.push({
    name: '计划亮灯', list: dataList.map(item => {
      return { name: item.point_time, value: `${BASE_TIME} ${item.k_time || '00:00'}` }
    })
  })
  chartDataList.value.push({
    name: '实际亮灯', list: dataList.map(item => {
