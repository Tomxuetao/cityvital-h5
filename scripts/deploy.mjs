...
const pkgName = 'city-ways'
const tempName = 'temp'
const backName = 'back'
const targetDir = '/data/front'
const timestamp = Date.now()

const build = () => execa('pnpm', ['build:city-ways'], {
  stdio: 'inherit'
})

const update = async (host) => await execa('ssh', [`root@${host}`, `mv ${targetDir}/${tempName} ${targetDir}/${pkgName}`], { stdio: 'inherit' })

const push = async (host) => await execa('rsync', ['-ah', '--progress', `packages/${pkgName}/${pkgName}/`, `root@${host}:${targetDir}/${tempName}`], { stdio: 'inherit' })
